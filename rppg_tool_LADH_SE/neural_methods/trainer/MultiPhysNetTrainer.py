...
from neural_methods.model.MultiPhysNet import (
    PhysNet_padding_Encoder_Decoder_MAX as MultiPhysNet
)
from neural_methods.model.ReMoPhysNet import (
    PhysNet_padding_Encoder_Decoder_MAX as RGMultiPhysNet
)
...
class MultiPhysNetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.MODEL.NAME == "ReMoPhysNet":
            self.model = RGMultiPhysNet(
                frames=config.MODEL.MultiPhysNet.FRAME_NUM).to(self.device)
        elif config.MODEL.NAME == "MultiPhysNet":
            self.model = MultiPhysNet(
                frames=config.MODEL.MultiPhysNet.FRAME_NUM).to(self.device)
        else:
            raise ValueError(
                f"Unsupported model for MultiPhysNetTrainer: {config.MODEL.NAME}")
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            if not self.config.TEST.USE_LAST_EPOCH: 
                ...
                with open ('/gpfs/home/zhangaofeng/LeiRZ/FusionVitals-main/rppg_tool_LADH/loss.csv', 'a', newline='') as csvfile:
    ...
    def test(self, data_loader):
        ...
        print('')
        file_exists = os.path.isfile('/gpfs/home/zhangaofeng/LeiRZ/FusionVitals-main/rppg_tool_LADH/result.csv')
        with open('/gpfs/home/zhangaofeng/LeiRZ/FusionVitals-main/rppg_tool_LADH/result.csv', 'a', newline='') as csvfile:
        ...
        # write data
            if self.config.TOOLBOX_MODE != "only_test":
                ...
                with open("/gpfs/home/zhangaofeng/LeiRZ/MAE.csv", 'a', newline='') as csvf:
